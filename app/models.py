...
class Role(db.Model, RoleMixin):
    """ Stores the various User's roles (for use in the relational table between Roles and Users)
        (e.g., "User," "Admin,")
        TODO: Switch to keys in keys.py for consistency. """
    ...
    def write(self, fref):
        """ Write this data structure to file reference (as a key'd text file). 'name' comes from filename. """
        fref.write("description=" + self.description + "\n")
...
class User(db.Model, UserMixin):
    ...
    def write(self, fref):
        """ Write this data structure to file reference (as a key'd text file). 'name' comes from filename.
        TODO: Switch to keys in keys.py for consistency. """
        fref.write("email=" + self.email + "\n")
        fref.write("password=" + self.password + "\n")
        fref.write("active=" + str(self.active) + "\n")
        fref.write("created=" + str(self.created) + "\n")
        fref.write("confirmed_at=" + str(self.confirmed_at) + "\n")
        fref.write("last_login_at=" + str(self.last_login_at) + "\n")
        fref.write("last_login_ip=" + self.last_login_ip + "\n")
        fref.write("current_login_ip=" + self.current_login_ip + "\n")
        fref.write("login_count=" + str(self.login_count) + "\n")
        fref.write("roles=")
        for index in range(len(self.roles)):
            if (index > 0):
                fref.write(",")
            fref.write(self.roles[index].name)
        fref.write("\n")
        fref.write("questions=")
        for index in range(len(self.questions)):
            if (index > 0):
                fref.write(",")
            fref.write(self.questions[index].classID)
        fref.write("\n")
    ...
    def wrote_question(self,question):
        self.questions.append(question.classID)
...
class ClassInfo(db.Model):
    ...
    def write(self, fref):
        """ Write this data structure to file reference (as a key'd text file). 'classAbbr' comes from folder name.
        TODO: Switch to keys in keys.py for consistency. """
        fref.write("description=" + self.longName + "\n")
        fref.write("startID=" + str(self.startingID) + "\n")
        fref.write("currentID=" + str(self.currentID) + "\n")
...
class Question(db.Model):
    ...
    @staticmethod
    def getAllByClassAndQuiz(classAbbr, quiz):
        """ Retrieve a question from the database by raw id. """
        questions = None
        questions = Question.query.filter_by(classAbbr=classAbbr, quiz=quiz)
        if ((questions == None) or (questions.count() == 0)):
            return None
    ...
    def write(self, fref):
        """ Write this data structure to file reference (as a key'd text file).
        'classAbbr' and 'quiz' comes from folder names.
        TODO: Switch to keys in keys.py for consistency. """
        fref.write(app.keys.QUESTION_CLASSID+"=" + str(self.classID) + "\n")
        fref.write(app.keys.QUESTION_CREATED+"=" + str(self.created) + "\n")
        fref.write(app.keys.QUESTION_MODIFIED+"=" + str(self.modified) + "\n")
        if (self.tags):
            fref.write("tags=" + self.tags + "\n")
        if (self.tagsComments):
            fref.write("tagsComments=" + self.tagsComments + "\n")
        if (self.instructions):
            fref.write("instructions=" + self.instructions + "\n")
        if (self.instructionsComments):
            fref.write("instructionsComments=" + self.instructionsComments + "\n")
        if (self.question):
            fref.write("question=" + self.question + "\n")
        if (self.questionComments):
            fref.write("questionComments=" + self.questionComments + "\n")
        if (self.examples):
            fref.write("examples=" + self.examples + "\n")
        if (self.examplesComments):
            fref.write("examplesComments=" + self.examplesComments + "\n")
        if (self.hints):
            fref.write("hints=" + self.hints + "\n")
        if (self.hintsComments):
            fref.write("hintsComments=" + self.hintsComments + "\n")
        if (self.answer):
            fref.write("answer=" + self.answer + "\n")
        if (self.answerComments):
            fref.write("answerComments=" + self.answerComments + "\n")
...
class Image(db.Model):
    ...
    def getByName(filenameIn):
        ...
        if ((images == None) or (images.count() == 0)):
            flash("Couldn't find any images in the database with filename: %s" % filenameIn)
            return None
        if (images.count() > 1):
            flash("Found more than one image (%d) in the database with filename: %s" % filenameIn)
        return images.one()
    ...
    @staticmethod
    def getAllByClass(classAbbr):
        images = Image.query.filter_by(classAbbr=classAbbr)
        return images.all()
